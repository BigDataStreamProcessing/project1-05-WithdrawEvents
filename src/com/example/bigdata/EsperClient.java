        ...
        while (System.currentTimeMillis() < startTime + (1000L * howLongInSec)) {
            for (int i = 0; i < noOfRecordsPerSec; i++) {
                ...
                record = Format.toJson()
                        .set("cardnumber", () -> faker.business().creditCardNumber())
                        .set("cardtype", () -> faker.business().creditCardType())
                        .set("amount", () -> String.valueOf(10 * faker.number().numberBetween(5, 100)))
                        .set("city", () -> faker.address().city())
                        .set("country", () -> faker.address().country())
                        .set("ets", eTimestamp::toString)
                        .set("its", iTimestamp::toString)
                        .build().generate();
                runtime.getEventService().sendEventJson(record, "WithdrawEvent");
            }
            waitToEpoch();
        }
    }
    ...
    private static EPCompiled getEPCompiled(Configuration config) {
        ...
        try {
            epCompiled = compiler.compile("""
                    @public @buseventtype create json schema WithdrawEvent(cardnumber string, cardtype string, amount int, city string, country string, ets string, its string);
                    @name('answer') SELECT cardnumber, cardtype, amount, city, country, ets, its from WithdrawEvent#ext_timed(java.sql.Timestamp.valueOf(its).getTime(), 3 sec);""", compilerArgs);
        }
